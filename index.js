...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.4wie4.mongodb.net/?retryWrites=true&w=majority`;
...
const run = async () => {
  try {
    ...
    const productCollenction = client.db("industo").collection("products");
    const userCollenction = client.db("industo").collection("users");
    const reviewCollenction = client.db("industo").collection("reviews");
    const orderCollenction = client.db("industo").collection("orders");
    const paymentCollenction = client.db("industo").collection("payments");
...
app.listen(port, () => {
  console.log(`industo app listening on port ${port}`);
});
